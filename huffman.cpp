...
void huffman::createMinHeap() {
    ...
    inFile.get(id);
// inFile: This is typically an object of the ifstream class, representing an input file stream.
// .get(id): This member function reads a single character from the input file stream and stores it in the variable specified by id.

    //Incrementing frequency of characters that appear in the input file
    // In C++, !infile.eof() is a condition often used to check whether the end-of-file (EOF) marker has been reached in an input file stream
    while (!inFile.eof()) {
        arr[id]->freq++;
        inFile.get(id);
    }
    inFile.close(); // close the file
    // It's good practice to explicitly close a file when you are done using it. While the file stream is often closed automatically when the program terminates, explicitly closing it can help ensure that resources are released promptly, especially in more complex programs.
...
void huffman::getTree() {
    ...
    for(int i = 0; i < size; i++) {
        ...
        buildTree(aCode, hCodeStr); // line 42
    }
    inFile.close();
}

void huffman::compress() {
    createMinHeap(); // line 61
    createTree(); //line 86
    createCodes(); // line 107
    saveEncodedFile(); //line 112
}

void huffman::decompress() {
    getTree(); // line 215
    saveDecodedFile(); //line 168
}